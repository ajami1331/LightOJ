...
bool grahamScan( int n ) {
    ...
    hullIndex = 3;
    for( int i=3; i<n; i++ ) {
        cerr << ccw( hull[hullIndex - 2], hull[hullIndex - 1], points[ i ] ) << "\n";
        while( hullIndex - 2 >= 0 && ccw( hull[hullIndex - 2], hull[hullIndex - 1], points[ i ] ) != -1 ) {
            hullIndex--;
            cerr << ccw( hull[hullIndex - 2], hull[hullIndex - 1], points[ i ] ) << "\n";
            cerr << hullIndex << " " << hullIndex - 2 << " " << hullIndex - 1 << "\n";
        }
...
int main() {
    ...
    for( int cs=1; cs<=t; cs++ ) {
        ...
        for( int i=1; i<n; i++ ) {
            while( points[pointIndex - 1] == points[ i ] && i<n ) i++;
            points[ pointIndex++ ] = points[i];
        }
        printf( "Case %d: ", cs );
        if( grahamScan( pointIndex ) ) {
            printf( "%.12f\n", solve() );
        } else {
            printf( "0\n" );
        }
