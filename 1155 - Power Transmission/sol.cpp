...
int main() {
	...
	while( t-- && ~scanf( "%d", &n ) ) {
		...
		for( int i=1; i<=n; i++ ) {
			scanf( "%d", &cap );
			flow_graph.add_edge( i, i + n, cap, false );
		}
		scanf( "%d", &m );
		for( int i=1; i<=m; i++ ) {
			scanf( "%d %d %d", &u, &v, &cap );
			flow_graph.add_edge( u + n, v, cap, false );
		}
		scanf( "%d %d", &b, &d );
		while( b-- ) {
			scanf( "%d", &u );
			flow_graph.add_edge( 0, u, INF, false );
			//flow_graph.graph[ u ][ u + n ] = INF;
		}
		while( d-- ) {
			scanf( "%d", &u );
			flow_graph.add_edge( u + n, ( 2 * n ) + 1, INF, false );
			//flow_graph.graph[ u ][ u + n ] = INF;
		}
		printf( "Case %d: %d\n", ++cs, flow_graph.max_flow( 0, 2 * n + 1 ) ); 
	}	
	return 0;
}
